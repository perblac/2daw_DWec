...
export function UsuarioFunction(nombre, publicaciones = []) {
  this._nombre = nombre;
  ...
  this.getComentarios = function (mensajes) {
    ...
    for (let publicacion of this._publicaciones) {
      const coments = {};
      const selectMensajes = mensajes.filter(
        (mensaje) => mensaje.postId == publicacion
      );
      coments[publicacion] = selectMensajes.map((mensaje) => {
        return {
          nombre: mensaje.name,
          email: mensaje.email,
  ...
  this.printComentarios = function printComentarios() {
    let str = "";
    str +=
      this.publicaciones.length < 1
        ? "."
        : this.publicaciones.length < 2
        ? " con el número "
        : " con los números ";
    for (let indice in this.publicaciones) {
      let numero = this.publicaciones[indice];
      if (indice == this.publicaciones.length - 1) {
        str += " y " + numero;
      } else if (indice == 0) {
        str += numero;
      } else {
    ...
    console.log(
      `El usuario ${this.nombre} tiene ${this.publicaciones.length} publicaciones${str}`
    );
    for (let publicacion of this.publicaciones) {
      console.log(`Los comentarios de la publicación ${publicacion} son:`);
      const arrayComentarios = this.getComentarios(mensajes).filter(
        (mensaje) => Object.keys(mensaje) == publicacion
      );
      for (let comentario of Array.from(arrayComentarios)[0][publicacion]) {
        console.log(
          `- ${comentario.mensaje} \n (por: ${comentario.nombre} [${comentario.email}])`
        );
      }
    }
  };
...
export class UsuarioClass {
  ...
  set publicacion(arrayIds) {
    this._publicaciones = arrayIds;
  }
  addPublication(id) {
    this._publicaciones.push(id);
  }
  getComentarios(mensajes) {
    const comentarios = [];
    for (let publicacion of this._publicaciones) {
      const coments = {};
      const selectMensajes = mensajes.filter(
...
export const UsuarioLiteral = {
  init: function(nombre, publicaciones = []) {
    this._nombre = nombre;
    this._publicaciones = publicaciones;
  },
  get nombre(){
