...
export function Usuario(nombre, publicaciones = []) {
    this._nombre = nombre;
    this._publicaciones = publicaciones;
    this.addPublication = function (id) {
        this._publicaciones.push(id);
    }
    ...
    Object.defineProperty(this, 'publicaciones', {
        get: function() {
            return this._publicaciones;
        },
        set: function(arrayIds) {
            this._publicaciones = arrayIds;
        },
    });
    this.getComentarios = function(mensajes) {
        const comentarios = [];
        for (let publicacion of this._publicaciones) {
            const coments = {};
            const selectMensajes  = mensajes.filter((mensaje)=> mensaje.postId == publicacion);
            coments[publicacion] = selectMensajes.map((mensaje) => {return {nombre: mensaje.name, email: mensaje.email, mensaje: mensaje.body,};});
            comentarios.push(coments);
        }
        return comentarios;
    }
}